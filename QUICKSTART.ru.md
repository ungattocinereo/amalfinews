...
**Amalfi Events Intelligence** - это умный переводчик событий:
- 🇮🇹 Собирает события с 11 итальянских сайтов
- 🤖 Фильтрует через DeepSeek AI (только для туристов)
- 🌍 Переводит на английский
- 📱 Отправляет вам в Telegram для модерации
- 💾 Сохраняет в базу данных

**Публикация - ваша забота!** Система дает вам готовые переводы.
...
### 3️⃣ Настройте API ключи (всего 2!)
...
**Заполните ТОЛЬКО 2 обязательных параметра:**
...
> 💡 **WordPress не нужен!** Система будет собирать и переводить события,
> отправлять их вам в Telegram. Публикация - на ваше усмотрение.
...
1. **Откройте веб-интерфейс** - там всё понятно и красиво
2. **Подождите первого запуска** - в 04:05 утра (или запустите вручную)
3. **Проверьте Telegram** - бот пришлёт переведённые события
4. **Одобрите события** - через кнопки в Telegram
5. **Используйте переводы** - скопируйте и опубликуйте куда нужно
6. **Или экспортируйте из БД** - все события в SQLite базе
